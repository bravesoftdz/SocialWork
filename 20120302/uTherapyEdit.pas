...
begin
  ...
  frmDoJob.ShowModal;
  dbMain.GetProgramPatientList(adoSchedule.FieldByName('ScheduleID').AsInteger, cxListProgramPatient.Items);
end;

procedure TfrmTherapyEdit.btnPrintClick(Sender: TObject);
begin
  frmTherapyPrint.FScheduleID := adoSchedule.FieldByName('ScheduleID').AsInteger;
...
begin
  ...
  try
    ...
    begin
      adoSchedule.Append;
      adoSchedule.FieldByName('HospitalID').AsInteger := oConfig.User.HospitalID;
      adoSchedule.FieldByName('WardID').AsInteger := FWardID;
    end
    else
      adoSchedule.Edit;

    adoSchedule.FieldByName('ProgramID').AsInteger := dbMain.GetProgramID(oConfig.User.HospitalID, cboProgram.Text);
    ...
    adoSchedule.FieldByName('StartTime').AsDateTime := oGlobal.DateAndTimeToDateTime(cxDateStart.date, cxTimeStart.Time);
    adoSchedule.FieldByName('FinishTime').AsDateTime := oGlobal.DateAndTimeToDateTime(cxDateStart.date, cxTimeFinish.Time);

    adoSchedule.FieldByName('SDate').AsString := oGlobal.DateToString(cxDateStart.Date);
    adoSchedule.FieldByName('FDate').AsString := oGlobal.DateToString(cxDateStart.Date);
    adoSchedule.FieldByName('STime').AsString := oGlobal.TimeToStringHN(cxTimeStart.Time);
    adoSchedule.FieldByName('FTime').AsString := oGlobal.TimeToStringHN(cxTimeFinish.Time);

    adoSchedule.FieldByName('ChairmanIDs').AsString := dbMain.GetChairmanIDs(oConfig.User.HospitalID, cxBEChairman.Text);
    adoSchedule.FieldByName('ChairmanNames').AsString := cxBEChairman.Text;
    adoSchedule.FieldByName('LocationIDs').AsString := dbMain.GetLocationIDs(oConfig.User.HospitalID, cxBELocation.Text);

    adoSchedule.FieldByName('ChairmanOpinion').AsString := trim(memoChairmanOpinion.Text);
    adoSchedule.FieldByName('WorkerID').AsString := dbMain.GetUserID(oConfig.User.HospitalID, cboWorker.Text);
    adoSchedule.FieldByName('WorkerOpinion').AsString := trim(memoWorkerOpinion.Text);
    adoSchedule.FieldByName('DoctorID').AsString := dbMain.GetDoctorID(oConfig.User.HospitalID, cboDoctor.Text);
    adoSchedule.FieldByName('DoctorOpinion').AsString := trim(memoDoctorOpinion.Text);
...
procedure TfrmTherapyEdit.btnShortKeyClick(Sender: TObject);
begin
...
begin
  ...
  oGlobal.SetListFromText(cxBEChairman.Text, cDelimeter, frmSelectList.ListBoxSelected.Items);
  dbMain.GetUserList(oConfig.User.HospitalID, frmSelectList.ListBoxAll.Items, False);

  if frmSelectList.ShowModal = mrOK then
  begin
    oGlobal.SetTextFromList(frmSelectList.ListBoxSelected.Items, cDelimeter, sTemp);
...
begin
  frmSelectList.ListBoxAll.Items.Clear;
  frmSelectList.ListBoxSelected.Items.Clear;
  oGlobal.SetListFromText(cxBELocation.Text, cDelimeter, frmSelectList.ListBoxSelected.Items);
  dbMain.GetLocationList(oConfig.User.HospitalID, frmSelectList.ListBoxAll.Items);
...
begin
  ...
  if dbMain.isExistScheduleInProgramPatient(adoSchedule.FieldByName('ScheduleID').AsInteger) then
  begin
...
begin
  ...
  if dbMain.getProgramList(oConfig.User.HospitalID, FWardID, cboProgram.Properties.Items, False) then
  begin
  ...
  dbMain.GetUserList(oConfig.User.HospitalID, cboWorker.Properties.Items, False);
  ...
  begin
    ...
    cxDateStart.EditText := oGlobal.DateToString(adoSchedule.FieldByName('StartTime').AsDateTime);
    ...
    chkNotFixed.Checked := adoSchedule.FieldByName('Options').AsInteger = cAllDayEvent;
    chkDone.Checked := adoSchedule.FieldByName('State').AsInteger = cJobDone;
    cboWorker.ItemIndex := cboWorker.Properties.Items.IndexOf(adoSchedule.FieldByName('WorkerName').AsString);
    cboDoctor.ItemIndex := cboDoctor.Properties.Items.IndexOf(adoSchedule.FieldByName('DoctorName').AsString);
    cxBELocation.Text := dbMain.GetLocationNames(oConfig.User.HospitalID, adoSchedule.FieldByName('LocationIDs').AsString);
    cxBEChairman.Text := dbMain.GetChairmanNames(oConfig.User.HospitalID, adoSchedule.FieldByName('ChairmanIDs').AsString);
    txtContent.Text :=adoSchedule.FieldByName('Content').AsString;
    memoChairmanOpinion.Lines.Text := adoSchedule.FieldByName('ChairmanOpinion').AsString;
    memoWorkerOpinion.Lines.Text := adoSchedule.FieldByName('WorkerOpinion').AsString;
    memoDoctorOpinion.Lines.Text := adoSchedule.FieldByName('DoctorOpinion').AsString;
  end;
end;
