object frmRatingMgr: TfrmRatingMgr
  ...
  object adoInOut: TADOQuery
    ...
    SQL.Strings = (
      ...
      ' where I.InOutID in (select distinct InOutID'
      
        '                       from ProgramPatients as PP inner join Sch' +
        'edule as S on S.ScheduleID=PP.ScheduleID and S.SDate like '#39'2011%' +
        #39')'
      ''
      '/*'
      'select Patients.PatientName as PatientName'
      '       ,Patients.Sex as Sex'
      ...
      '        end as StateName'
        ...
        '       ,(select UserName from users where hospitalid=0 and useri' +
        'd=inout.UserID and Worker=1) as WorkerName'
      
        '       ,(select UserName from users where hospitalid=0 and useri' +
        'd=inout.DoctorID and Doctor=1) as DoctorName'
      
        '       ,(select WardName from Wards where wards.wardid=inout.war' +
        'did) as WardName'
      
        '  from inout inner join patients on inout.hospitalID=patients.ho' +
        'spitalID and inout.patientID=patients.PatientID'
      ' where state=:state'
      '   and WardID=:WardID'
