object frmInTypeMgr: TfrmInTypeMgr
  Left = 0
  Top = 0
  Caption = #51077#50896#54805#53468
  ClientHeight = 416
  ClientWidth = 410
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  Padding.Left = 15
  Padding.Top = 15
  Padding.Right = 15
  Padding.Bottom = 15
  OldCreateOrder = False
  OnClose = FormClose
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object cxGrid1: TcxGrid
    Left = 15
    Top = 15
    Width = 273
    Height = 386
    Align = alLeft
    TabOrder = 0
    object cxGInType: TcxGridDBTableView
      OnCustomDrawCell = cxGInTypeCustomDrawCell
      DataController.DataSource = dsInType
      DataController.Summary.DefaultGroupSummaryItems = <>
      DataController.Summary.FooterSummaryItems = <>
      DataController.Summary.SummaryGroups = <>
      OptionsData.Deleting = False
      OptionsData.Editing = False
      OptionsData.Inserting = False
      OptionsView.ColumnAutoWidth = True
      OptionsView.GroupByBox = False
      OptionsView.Indicator = True
      Styles.Inactive = dbMain.cxSelection
      object cxGInTypeInTypeName: TcxGridDBColumn
        Caption = #51077#50896#54805#53468
        DataBinding.FieldName = 'InTypeName'
        HeaderAlignmentHorz = taCenter
        Options.Editing = False
        Options.Filtering = False
        Options.Sorting = False
        Width = 201
      end
      object cxGInTypeUsed: TcxGridDBColumn
        DataBinding.FieldName = 'Used'
        HeaderAlignmentHorz = taCenter
        Options.Editing = False
        Options.Filtering = False
        Options.Sorting = False
        Width = 34
      end
    end
    object cxGrid1Level1: TcxGridLevel
      GridView = cxGInType
    end
  end
  object btnInTypeAppend: TcxButton
    Left = 305
    Top = 34
    Width = 75
    Height = 50
    Caption = #52628#44032
    Glyph.Data = {
      
      88FF000000000000000000000000000000000000000000000000}
    Layout = blGlyphTop
    LookAndFeel.SkinName = 'Caramel'
    SpeedButtonOptions.Transparent = True
    TabOrder = 1
    OnClick = btnInTypeAppendClick
  end
  object btnInTypeUpdate: TcxButton
    Left = 305
    Top = 90
    Width = 75
    Height = 50
    Caption = #49688#51221
    Glyph.Data = {
      
      88FF000000000000000000000000000000000000000000000000}
    Layout = blGlyphTop
    LookAndFeel.SkinName = 'Caramel'
    SpeedButtonOptions.Transparent = True
    TabOrder = 2
    OnClick = btnInTypeUpdateClick
  end
  object adoInType: TADOQuery
    Connection = dbMain.adoSocialWorkDB
    CursorType = ctStatic
    Parameters = <
      item
        Name = 'hospitalID'
        DataType = ftWideString
        Size = 1
        Value = '1'
      end>
    SQL.Strings = (
      
        'select * from InTypes where hospitalID=:hospitalID order by InTy' +
        'peName')
    Left = 90
    Top = 104
  end
  object dsInType: TDataSource
    DataSet = adoInType
    OnDataChange = dsInTypeDataChange
    Left = 150
    Top = 104
  end
end
