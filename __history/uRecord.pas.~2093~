...
uses
  ...
  dxSkinXmas2008Blue, cxCustomData, cxFilter, cxData,
...
var
  nHospitalID, nWardID, nBohumID: integer;
  sHospitalID: string;
begin
...
//    nInOut := -1;
  ...
  adoInOut.SQL.Add('       ,(select WardName from Wards where WardID = W.WardID) as WardName');
  adoInOut.SQL.Add('       ,(select count(*) from intake');
  adoInOut.SQL.Add('                        where I.InOutID=intake.InOutID) as intake');
  adoInOut.SQL.Add('       ,(select count(*) from survey1');
  adoInOut.SQL.Add('                         where I.InOutID=survey1.InOutID) as survey1');
  adoInOut.SQL.Add('       ,(select count(*) from survey2');
  adoInOut.SQL.Add('                        where I.InOutID=survey2.InOutID) as survey2');
  adoInOut.SQL.Add('       ,(select count(*) from counseling');
  adoInOut.SQL.Add('                        where I.InOutID=counseling.InOutID) as counseling');
  adoInOut.SQL.Add('       ,(select count(*) from training');
  adoInOut.SQL.Add('                        where I.InOutID=training.InOutID) as training');
  adoInOut.SQL.Add('       ,(select count(*) from visit');
  adoInOut.SQL.Add('                        where I.InOutID=visit.InOutID) as Visit');
  adoInOut.SQL.Add('  from vinout AS I');
  adoInOut.SQL.Add('    	 INNER JOIN WardHistory AS W');
  adoInOut.SQL.Add('  				ON I.InOutID=W.InOutID');
  adoInOut.SQL.Add(' 	  	   AND W.WardDate = (select max(WardDate)');
  adoInOut.SQL.Add('                             from WardHistory');
  adoInOut.SQL.Add('                            where W.InOutID=InOutID');
  adoInOut.SQL.Add('                              and WardDate <= ''9999-99-99'')');

  adoInOut.SQL.Add(' where hospitalid=' + sHospitalID);

  if cboWard.ItemIndex <> 0 then
    adoInOut.SQL.Add('   and WardID=' + inttostr(nWardID));

  if cboBohum.ItemIndex <> 0 then
    adoInOut.SQL.Add('   and BohumID=' + inttostr(nBohumID));

  if rIn.Checked then
    adoInOut.SQL.Add('   and OutDate <= ''''')
  else if rOut.Checked then
    adoInOut.SQL.Add('   and OutDate > ''''');

//  if nInOut <> -1 then
//    adoInOut.SQL.Add('   and state=' + inttostr(nInOut));

  if cboSearchCondition.ItemIndex = 0 then
  begin
    adoInOut.SQL.Add('   and PatientName LIKE ''%' + trim(txtSearch.Text) + '%''');
    adoInOut.SQL.Add(' order by PatientName, InDate desc');
  ...
  begin
    adoInOut.SQL.Add('   and PatientID LIKE ''%' + trim(txtSearch.Text) + '%''');
    adoInOut.SQL.Add(' order by PatientID, InDate desc');
...
begin
  ...
  dbMain.RemakeCounselingOrder(nInOutID);
...
begin
  qrlblCounselingTitle.Caption := cxTabCounseling.Caption;
...
begin
  ...
  chkSurvey1Religion4.Checked := False;
...
begin
  ...
  try
    cxTabIntake.Caption := dbMain.GetRecordCustomName(cRecordIntake);
    cxTabSurvey1.Caption := dbMain.GetRecordCustomName(cRecordSurvey1);
    cxTabSurvey2.Caption := dbMain.GetRecordCustomName(cRecordSurvey2);
    cxTabCounseling.Caption := dbMain.GetRecordCustomName(cRecordCounseling);
    cxTabTraining.Caption := dbMain.GetRecordCustomName(cRecordTraining);
    cxTabVisit.Caption := dbMain.GetRecordCustomName(cRecordVisit);
  except
    oGlobal.Msg('기록지명 정보를 가져오지 못했습니다!');
    FClose := True;
  end;
...
begin
  ...
  qrlblSurvey1Title.Caption := cxTabSurvey1.Caption;
  ...
  begin
    ...
    chkSurvey1Religion3.Checked := (adoSurvey1.FieldByName('Religion')
        .AsInteger = 3);
    chkSurvey1Religion4.Checked := (adoSurvey1.FieldByName('Religion')
        .AsInteger = 4);
  end;
  ...
  if qrMemoSurvey1ETC.Lines.Count > 1 then
    qrMemoSurvey1ETCLine.Top := 25
  else
    qrMemoSurvey1ETCLine.Top := 1;
...
begin
  ...
  qrlblSurvey2Title.Caption := cxTabSurvey2.Caption;
...
begin
  ...
  qrlblTrainingTitle.Caption := cxTabTraining.Caption;
...
begin
  ...
  qrlblVisitTitle.Caption := cxTabVisit.Caption + ' ' +
    adoVisit.FieldByName('Seq').AsString;
  ...
  if QRVisit.TextHeight(QRVisit.Font, 'AAA') * qrMemoVisitComment.Lines.Count >
    qrMemoVisitCommentTitle.Height then
    qrMemoVisitCommentLine.Top := 15
  else
    qrMemoVisitCommentLine.Top := 5;
end;
...
begin
  ...
  qrlblCounselingTitle.Caption := cxTabCounseling.Caption  + ' ' +
    adoCounseling.FieldByName('Seq').AsString;
...
procedure TfrmRecord.QRIntakeBeforePrint(Sender: TCustomQuickRep;
  var PrintReport: Boolean);
begin
  qrlblIntakeTitle.Caption := cxTabIntake.Caption;
  ...
  if QRIntake.TextHeight(QRIntake.Font, 'AAA') * QRMemoFamilyName.Lines.Count >
    qrMemoRelationshipTitle.Height then
    qrMemoRelationshipLine.Top := 80
  else
    qrMemoRelationshipLine.Top := 70;
  ...
  if QRIntake.TextHeight(QRIntake.Font, 'AAA')
    * qrMemoWorkerOpinion.Lines.Count > qrMemoWorkerOpinionTitle.Height then
    qrmemoWorkerOpinionLine.Top := 45
  else
    qrmemoWorkerOpinionLine.Top := 35;
...
procedure TfrmRecord.txtSearchKeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if not FLoaded then
    Exit;
