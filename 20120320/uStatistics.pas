...
uses
  ...
  dxPSCore, dxPScxCommon, dxPScxGrid7Lnk, cxCheckBox, dxSkinBlack, dxSkinBlue,
...
type
  ...
  private
    ...
    FPrevCount0: integer;
    FPrevCount1: integer;
    FCount0: Array of integer; // 일반 합계
    FCount1: Array of integer; // 작업 합계
...
begin
  ...
  begin
    ...
    AText := sD + inttostr(FCount0[nWeek - FMinWeek]) + sD;
  end;
end;
...
begin
  ...
  begin
    ...
    AText := sD + inttostr(FCount1[nWeek - FMinWeek]) + sD;
  end;
end;
...
begin
  ...
  SetLength(FCount0, 0);
  SetLength(FCount1, 0);
  SetLength(FCount0, FWeekCount);
  SetLength(FCount1, FWeekCount);
  ...
  oT.SQL.Add('SELECT SDate, FDate, sTime, ProgramTypeID');
  ...
  begin
    ...
    cxGError.DataController.SetValue(row, 1, oT.FieldByName('SDate').AsString);
    cxGError.DataController.SetValue(row, 2, oT.FieldByName('FDate').AsString);
    bError := CheckOvernight(oT.FieldByName('SDate').AsString, oT.FieldByName('FDate').AsString);
    if bError then
      inc(nErrorCnt);
  ...
  begin
    ...
    begin
      ...
      FCount0[nWeek - FMinWeek] := FCount0[nWeek - FMinWeek] + 1;
    end;

    if not VarIsNull(cxGShimsa.DataController.GetValue(n, col작업)) then
    begin
      nCol := cxGShimsa.DataController.GetValue(n, cxGShimsaWeek1.index);
      FCount1[nCol - FMinWeek] := FCount1[nCol - FMinWeek] + 1;
    end;
  end;
  ...
  begin
    ...
    if FPrevCount0 + FCount0[0] > 2 then
      FCount0[0] := 2 - FPrevCount0;

    if FPrevCount1 + FCount1[0] > 5 then
      FCount1[0] := 5 - FPrevCount1;
  ...
  for n := 0 to FWeekCount - 1 do
  begin
    FCountSum0 := FCountSum0 + FCount0[n];
    FCountSum1 := FCountSum1 + FCount1[n];
  end;
...
begin
  ...
  begin
    ...
    Add('select * from');
    Add('(');
    Add('select vInOut.InOutID');
    Add('       , vInOut.PatientID');
    Add('       , vInOut.PatientName');
    Add('       , vInOut.Birthday');
    Add('       , vInOut.Sex');
    Add('       , vInOut.InDate');
    Add('       , vInOut.OutDate');
    Add('       , vInOut.BohumID');
    Add('       , vInOut.BohumName');
    Add('       , WardID');
    Add('       ,(select WardName from Wards where WardId=W.WardId) as WardName');
    Add('       ,(select count(InOutID)');
    Add('           from intake');
    Add('          where InOutID=vInOut.inoutid');
    ...
    Add('       ,(select count(InOutID)');
    Add('           from vPPSchedule');
    Add('          where InOutID=vInOut.inoutid and SDate between ' + sStartDate + ' and ' + sFinishDate + ') as PP');

    Add('       ,(select count(InOutID)');
    Add('           from vPPSchedule');
    Add('          where InOutID=vInOut.inoutid and ProgramTypeID=1 and SDate between ' + sStartDate + ' and ' + sFinishDate + ') as PP1');

    Add('       ,(select count(InOutID)');
    Add('           from vPPSchedule');
    Add('          where InOutID=vInOut.inoutid and ProgramTypeID=2 and SDate between ' + sStartDate + ' and ' + sFinishDate + ') as PP2');

    Add('       ,(select count(InOutID)');
    Add('           from vPPSchedule');
    Add('          where InOutID=vInOut.inoutid and ProgramTypeID=6 and SDate between ' + sStartDate + ' and ' + sFinishDate + ') as PP5');

    Add('       ,(select count(InOutID)');
    Add('           from vPPSchedule');
    Add('          where InOutID=vInOut.inoutid and ProgramTypeID=3 and SDate between ' + sStartDate + ' and ' + sFinishDate + ') as PP3');

    Add('       ,(select count(InOutID)');
    Add('           from vPPSchedule');
    Add('          where InOutID=vInOut.inoutid and ProgramTypeID=4 and SDate between ' + sStartDate + ' and ' + sFinishDate + ') as PP4');

    Add('  from vInOut');
    Add('    	  INNER JOIN WardHistory AS W');
    Add('  			        ON W.InOutID=vInOut.InOutID');
    ...
    if cboSearchCondition.ItemIndex = 0 then
      ...
      Add('    and PatientID LIKE ''%' + trim(txtSearch.Text) + '%''');
