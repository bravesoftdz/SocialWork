...
uses
  ...
  StdCtrls, Messages, Graphics, Dialogs, cxClasses;
...
type
  ...
  public
    ...
    function GetAppTitle(nHospitalID: integer): string;
    ...
    function isProgramTypeLinkRecord(sProgramTypeName: string): Boolean;
    ...
    function GetProgramPatientList(nScheduleID: integer; oList: TStrings)
      : Boolean;
    function GetWardList(nHospitalID: integer; oList: TStrings; bAll: Boolean=False)
      : Boolean;
    function GetRelationshipList(nHospitalID: integer; oList: TStrings)
      : Boolean;
    function GetBohumList(oList: TStrings; bAll: Boolean=False): Boolean;
    ...
    function GetUserList(nHospitalID: integer; oList: TStrings; bAll: Boolean=False)
      : Boolean;
    ...
    function GetProgramCount(nInOutID: integer; SDate, FDate: string; nProgramTypeID: integer): integer;
    function GetRecordCustomName(nID: integer): string;
    ...
    function isDuplicatedShortkey(nHospitalID: integer; sKeyword: string)
      : Boolean;
    function isDuplicatedWardHistory(nInOutID: integer; sWardDate: string): Boolean;
    function isExistsInFamilies(nInOutID: integer): Boolean;
    function isExistsInOverNight(nInOutID: integer): Boolean;
    ...
    function isExistKeyword(nHospitalID: integer; sKeyword: string): Boolean;
    ...
    function isExistWardHistory(nInOutID: integer; sWardDate: string): Boolean; overload;
    function isExistWardHistory(nInOutID: integer; sWardDate, sWardID: string): Boolean; overload;
    function isExistWardHistoryLessThen(nInOutID: integer; sInDate, sWardDate: string): Boolean;
    function isExistInIntake(nInOutID: integer): Boolean;
    function isExistInSurvey1(nInOutID: integer): Boolean;
    function isExistInSurvey2(nInOutID: integer): Boolean;
    function isKeyword(nHospitalID: integer; sKeyword: string;
      var sContent: string): Boolean;
    function RemakeCounselingOrder(nInOutID: integer): Boolean;
    function RemakeTrainingOrder(nInOutID: integer): Boolean;
    function RemakeVisitOrder(nInOutID: integer): Boolean;
    function SaveNewPatient(oP: TNewPatientInfo): integer;
    function UpdatePatient(nHospitalID: integer; sPatientID, sPatientName, sSex, sBirthday: string): Boolean;
...
function TdbMain.GetProgramCount(nInOutID: integer; SDate, FDate: string;
  nProgramTypeID: integer): integer;
begin
  try
    adoTemp.SQL.Text
      := 'SELECT Count(distinct sDate) as Cnt' +
         '  FROM vPPSchedule' +
         ' WHERE InOutID=' + inttostr(nInOutID) +
         '   AND SDate BETWEEN ''' + SDate + ''' AND ''' + FDate + '''' +
         '   AND ProgramTypeID=' + inttostr(nProgramTypeID);
    adoTemp.Open;
    ...
    if adoTemp.IsEmpty then
      ...
      result := adoTemp.FieldByName('Cnt').AsInteger;
...
function TdbMain.isProgramTypeLinkRecord(sProgramTypeName: string): Boolean;
begin
  try
    with adoTemp.SQL do
    begin
    ...
    if adoTemp.IsEmpty then
      ...
      result := adoTemp.FieldByName('LinkRecord').ASBoolean;
...
begin
  ...
  try
    ...
    if not adoTemp.IsEmpty then
      sNames := sNames + '  ΐΜ»σ ' + inttostr(adoTemp.RecordCount) + 'Έν';
...
function TdbMain.GetRecordCustomName(nID: integer): string;
begin
  try
    with adoTemp.SQL do
    begin
    ...
    if adoTemp.IsEmpty then
    begin
      adoTemp.SQL.Text := 'SELECT * FROM RecordNameDefault WHERE ID=' + inttostr(nID);
      adoTemp.Open;
      if adoTemp.IsEmpty then
        result := NullString
      else
      ...
      result := adoTemp.FieldByName('CustomName').AsString;
...
function TdbMain.GetAppTitle(nHospitalID: integer): string;
var
  oT: TADOQuery;
begin
  oT := TADOQuery.Create(self);
  try
    ...
    if oT.IsEmpty then
      ...
      result := oT.FieldByName('AppTitle').AsString;
    oT.Close;
  end;
  oT.Free;
end;
...
function TdbMain.GetBohumList(oList: TStrings; bAll: Boolean=False): Boolean;
begin
  try
    oList.Clear;
...
function TdbMain.isDuplicatedWardHistory(nInOutID: integer;
  sWardDate: string): Boolean;
begin
  try
    adoTemp.SQL.Text := 'SELECT *' +
                        '  FROM WardHistory' +
                        ' WHERE InOutID=' + inttostr(nInOutID) +
                        '   AND WardDate=''' + sWardDate + '''';
...
function TdbMain.isExistInIntake(nInOutID: integer): Boolean;
begin
  try
    adoTemp.SQL.Text :=
      'SELECT TOP 1 * FROM Intake WHERE INOUTID=' + inttostr(nInOutID);
    adoTemp.Open;
  finally
    result := not adoTemp.IsEmpty;
...
function TdbMain.isExistInSurvey1(nInOutID: integer): Boolean;
begin
  try
    adoTemp.SQL.Text :=
...
function TdbMain.isExistInSurvey2(nInOutID: integer): Boolean;
begin
  try
    adoTemp.SQL.Text :=
...
function TdbMain.isExistKeyword(nHospitalID: integer;
  sKeyword: string): Boolean;
begin
  try
    adoTemp.SQL.Text := 'SELECT *' +
                        '  FROM Keyword' +
                        ' WHERE HospitalID=' + inttostr(nHospitalID) +
                        '   AND Keyword=''' + sKeyword + '''';
    adoTemp.Open;
  finally
    result := not adoTemp.IsEmpty;
...
function TdbMain.isExistsInFamilies(nInOutID: integer): Boolean;
begin
  try
    adoTemp.SQL.Text :=
...
function TdbMain.isExistsInOverNight(nInOutID: integer): Boolean;
begin
  try
    adoTemp.SQL.Text :=
      'SELECT TOP 1 * FROM OverNight WHERE INOUTID=' + inttostr(nInOutID);
    adoTemp.Open;
  finally
    result := not adoTemp.IsEmpty;
...
function TdbMain.isExistWardHistory(nInOutID: integer;
  sWardDate: string): Boolean;
begin
  try
    with adoTemp.SQL do
...
function TdbMain.isExistWardHistory(nInOutID: integer; sWardDate,
  sWardID: string): Boolean;
begin
  try
    with adoTemp.SQL do
    begin
...
function TdbMain.isExistWardHistoryLessThen(nInOutID: integer;
  sInDate, sWardDate: string): Boolean;
begin
  try
    with adoTemp.SQL do
    begin
...
function TdbMain.isHas7: Boolean;
begin
  try
    with adoTemp.SQL do
    begin
...
function TdbMain.RemakeCounselingOrder(nInOutID: integer): Boolean;
var
  n: integer;
begin
  with adoTemp.SQL do
  ...
  if adoTemp.Connection.InTransaction then
  begin
...
function TdbMain.RemakeTrainingOrder(nInOutID: integer): Boolean;
var
  n: integer;
begin
  with adoTemp.SQL do
...
function TdbMain.RemakeVisitOrder(nInOutID: integer): Boolean;
var
  n: integer;
begin
   with adoTemp.SQL do
...
begin
  ...
  try
    ...
    oT.FieldByName('OutDate').AsString := oP.oInOut.sOutDate;
    oT.FieldByName('OutTime').AsString := oP.oInOut.sOutTime;
...
//      (oP.oInOut.nHospitalID, oP.oInOut.sWardName);
    ...
    nInOutID := oT.FieldByName('InOutID').AsInteger;

    oT.SQL.Text := 'SELECT * FROM WardHistory WHERE InOutID=' + inttostr(nInOutID);
    oT.Open;

    oT.Append;
    oT.FieldByName('InOutID').AsInteger := nInOutID;
    oT.FieldByName('WardDate').AsString := oP.oInOut.sInDate;
    oT.FieldByName('WardID').AsInteger := dbMain.GetWardID
      (oP.oInOut.nHospitalID, oP.oInOut.sWardName);
    oT.Post;
...
function TdbMain.UpdatePatient(nHospitalID: integer; sPatientID, sPatientName,
  sSex, sBirthday: string): Boolean;
var
  oT: TADOQuery;
begin
  oT := TADOQuery.Create(self);
  ...
  try
    ...
    begin
      Clear;
      Add('UPDATE Patients');
      Add('   SET PatientName=''' + sPatientName + '''');
      Add('       ,Sex=''' + sSex + '''');
      Add('       ,Birthday=''' + sBirthday + '''');
      ADD(' WHERE HospitalID=' + inttostr(nHospitalID));
      ADD('   AND PatientID=''' + sPatientID + '''');
    end;
    oT.ExecSQL;
...
begin
  try
    ...
    begin
      adoTemp.SQL.Text :=
        'SELECT TOP 1 * FROM Visit WHERE INOUTID=' + inttostr(nInOutID);
      adoTemp.Open;
      if not adoTemp.IsEmpty then
        result := rtVisit;
    end;
