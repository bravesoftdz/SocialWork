object frmProgramMgr: TfrmProgramMgr
  Left = 0
  Top = 0
  Caption = #54532#47196#44536#47016#44288#47532
  ClientHeight = 463
  ClientWidth = 714
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  OldCreateOrder = False
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object btnUserAppend: TcxButton
    Left = 605
    Top = 72
    Width = 75
    Height = 50
    Caption = #52628#44032
    LookAndFeel.SkinName = 'Caramel'
    OptionsImage.Glyph.Data = {
      
      88FF000000000000000000000000000000000000000000000000}
    OptionsImage.Layout = blGlyphTop
    SpeedButtonOptions.Transparent = True
    TabOrder = 0
    OnClick = btnUserAppendClick
  end
  object btnUserUpdate: TcxButton
    Left = 605
    Top = 128
    Width = 75
    Height = 50
    Caption = #49688#51221
    LookAndFeel.SkinName = 'Caramel'
    OptionsImage.Glyph.Data = {
      
      88FF000000000000000000000000000000000000000000000000}
    OptionsImage.Layout = blGlyphTop
    SpeedButtonOptions.Transparent = True
    TabOrder = 1
    OnClick = btnUserUpdateClick
  end
  object Panel1: TPanel
    Left = 0
    Top = 0
    Width = 589
    Height = 463
    Align = alLeft
    BevelOuter = bvNone
    Padding.Left = 15
    Padding.Top = 15
    Padding.Right = 15
    Padding.Bottom = 15
    TabOrder = 2
    object Panel2: TPanel
      Left = 15
      Top = 15
      Width = 559
      Height = 31
      Align = alTop
      BevelOuter = bvNone
      TabOrder = 0
      object cxLabel1: TcxLabel
        Left = 0
        Top = 6
        Caption = #44396#48516
        Transparent = True
      end
      object cboProgram: TcxComboBox
        Left = 28
        Top = 4
        Properties.DropDownListStyle = lsFixedList
        TabOrder = 1
        OnClick = actSelectExecute
        Width = 121
      end
      object cxLabel2: TcxLabel
        Left = 156
        Top = 6
        Caption = #48337#46041
        Transparent = True
      end
      object cboWard: TcxComboBox
        Left = 184
        Top = 4
        Properties.DropDownListStyle = lsFixedList
        Properties.DropDownRows = 16
        TabOrder = 3
        OnClick = actSelectExecute
        Width = 121
      end
    end
    object cxG: TcxGrid
      Left = 15
      Top = 46
      Width = 559
      Height = 402
      Align = alClient
      TabOrder = 1
      object cxGProgram: TcxGridDBTableView
        Navigator.Buttons.CustomButtons = <>
        OnCustomDrawCell = cxGProgramCustomDrawCell
        DataController.DataSource = dsProgram
        DataController.Summary.DefaultGroupSummaryItems = <>
        DataController.Summary.FooterSummaryItems = <>
        DataController.Summary.SummaryGroups = <>
        OptionsData.Deleting = False
        OptionsData.Editing = False
        OptionsData.Inserting = False
        OptionsView.ColumnAutoWidth = True
        OptionsView.GroupByBox = False
        OptionsView.Indicator = True
        Styles.Inactive = dbMain.cxSelection
        object cxGProgramProgramTypeName: TcxGridDBColumn
          Caption = #44396#48516
          DataBinding.FieldName = 'ProgramTypeName'
          HeaderAlignmentHorz = taCenter
          Options.Editing = False
          Options.Filtering = False
          Options.Sorting = False
          Width = 104
        end
        object cxGProgramProgramName: TcxGridDBColumn
          Caption = #54532#47196#44536#47016#47749
          DataBinding.FieldName = 'ProgramName'
          HeaderAlignmentHorz = taCenter
          Options.Editing = False
          Options.Filtering = False
          Options.Sorting = False
          Width = 151
        end
        object cxGProgramUseAll: TcxGridDBColumn
          Caption = #51204#52404#49324#50857
          DataBinding.FieldName = 'UseAll'
          HeaderAlignmentHorz = taCenter
          Options.Editing = False
          Options.Filtering = False
          Options.Sorting = False
          Width = 55
        end
        object cxGProgramWardName: TcxGridDBColumn
          Caption = #48337#46041#51060#47492
          DataBinding.FieldName = 'WardName'
          HeaderAlignmentHorz = taCenter
          Options.Editing = False
          Options.Filtering = False
          Options.Sorting = False
          Width = 72
        end
        object cxGProgramRegDate: TcxGridDBColumn
          Caption = #46321#47197#51068
          DataBinding.FieldName = 'RegDate'
          HeaderAlignmentHorz = taCenter
          Options.Editing = False
          Options.Filtering = False
          Options.Sorting = False
          Width = 69
        end
        object cxGProgramUsed: TcxGridDBColumn
          DataBinding.FieldName = 'Used'
          HeaderAlignmentHorz = taCenter
          Options.Editing = False
          Options.Filtering = False
          Options.Sorting = False
          Width = 34
        end
        object cxGProgramColor: TcxGridDBColumn
          DataBinding.FieldName = 'Color'
          PropertiesClassName = 'TcxColorComboBoxProperties'
          Properties.Alignment.Horz = taRightJustify
          Properties.ColorValueFormat = cxcvInteger
          Properties.CustomColors = <>
          Properties.ReadOnly = True
          HeaderAlignmentHorz = taCenter
          Width = 35
        end
      end
      object cxGLevel1: TcxGridLevel
        GridView = cxGProgram
      end
    end
  end
  object adoProgram: TADOQuery
    Connection = dbMain.adoSocialWorkDB
    CursorType = ctStatic
    Parameters = <>
    SQL.Strings = (
      'select *'
      
        '       ,(select ProgramTypeName from ProgramTypes where ProgramT' +
        'ypes.ProgramTypeID=Programs.ProgramTypeID) as ProgramTypeName'
      
        '       ,(select WardName from Wards where Wards.WardID=Programs.' +
        'WardID) as WardName'
      
        'from programs where hospitalid=1 order by ProgramTypeName, Progr' +
        'amName')
    Left = 114
    Top = 144
  end
  object dsProgram: TDataSource
    DataSet = adoProgram
    Left = 182
    Top = 144
  end
  object ActionList1: TActionList
    Left = 58
    Top = 188
    object actSelect: TAction
      Caption = 'actSelect'
      OnExecute = actSelectExecute
    end
  end
end
